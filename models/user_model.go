...
type UserModel struct {
	Model
	UserName string `gorm:"column:userName;size:36;unique;not null;comment:用户名" json:"-"`
	Password string `gorm:"column:password;size:128;comment:密码" json:"-"`
	Avatar   string `gorm:"column:avatar;size:256;comment:头像" json:"avatar"`
	...
	Token    string `gorm:"column:token;size:64;comment:token" json:"-"`
